...
static jlong
Spake2Context_AllocNewContext(JNIEnv *env, jclass clazz, jint myRole, jbyteArray myName,
    ...
    struct spake2_ctx_st *ctx = SPAKE2_CTX_new(my_role, (uint8_t *) my_name, my_len,
                                               (uint8_t *) their_name, their_len);
...
static jbyteArray
Spake2Context_GenerateMessage(JNIEnv *env, jclass clazz, jlong ctxPtr, jbyteArray password) {
    ...
    int status = SPAKE2_generate_msg(ctx, msg, &msg_size, SPAKE2_MAX_MSG_SIZE, (uint8_t *) pswd,
                                     pswd_size);
...
static jbyteArray
Spake2Context_ProcessMessage(JNIEnv *env, jclass clazz, jlong ctxPtr, jbyteArray theirMessage) {
    struct spake2_ctx_st *ctx = (struct spake2_ctx_st *) ctxPtr;
    ...
    int status = SPAKE2_process_msg(ctx, key_material, &key_material_len, SPAKE2_MAX_KEY_SIZE,
                                    (uint8_t *) their_msg, their_msg_len);
...
JNICALL JNI_OnLoad(JavaVM *vm, void *reserved) {
    ...
    env->RegisterNatives(env->FindClass("io/github/muntashirakon/crypto/spake2/Spake2Context"),
                         methods_Spake2Context,
