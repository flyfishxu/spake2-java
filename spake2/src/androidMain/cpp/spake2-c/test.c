...
int main() {
    ...
    struct spake2_ctx_st *alice = SPAKE2_CTX_new(spake2_role_alice, kClientName,
                                                 sizeof(kClientName), kServerName,
                                                 sizeof(kServerName));
    struct spake2_ctx_st *bob = SPAKE2_CTX_new(spake2_role_bob, kServerName, sizeof(kServerName),
    ...
    int status = SPAKE2_generate_msg(alice, aMessage, &aMsgSize, SPAKE2_MAX_MSG_SIZE, kPassword,
                                     sizeof(kPassword));
    status += SPAKE2_generate_msg(bob, bMessage, &bMsgSize, SPAKE2_MAX_MSG_SIZE, kPassword,
                                  sizeof(kPassword));
    ...
    status = SPAKE2_process_msg(alice, aKey, &aKeyLen, sizeof(aKey), (uint8_t *) bMessage,
                                bMsgSize);
